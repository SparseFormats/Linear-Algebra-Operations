...
//---- JAD
int a_JADptr[ NZcolMax + 1];
int a_NZ_row[ ROWS ];
int a_row_order[ ROWS ];
...
float compareVec(int a[], int b[], int vecSize){
  int i;
  float sum = 0;
  for (i = 0; i < vecSize; i++){
      sum += pow ((a[i] - b[i]), 2);
...
void SpMV(int a_val[], int a_col[], int a_row[], int a_rowPtr[], int a_val_col_row[ NZrowMax + 1 ][ 3 ], int a_val_row[ ROWS ][ NZrowMax ], int a_index_row[ ROWS ][ NZrowMax + 1], val_col_node * a_row_list[], int a_row_max, int a_col_max, int b_vec_val[], int b_vec_index[], int b_vec_size, int vec_b[], int c[]){
  int i, j, k = a_col_max;
  
  if (a_col_max < b_vec_size){
    printf("BAD MATRIX DIMENSION, a_col :%d, b_size : %d\n", a_col_max, b_vec_size);
    return;
  }
  ...
  int vec_row[ a_row_max ], vec_res[ a_row_max];
  int p_i, p_i_pr;

  int* alaki = NULL;
  memcpy(&i,alaki,sizeof(int));
    for (i = 0; i < a_row_max; i++){
      //printf("%d\n",i);
	...
	Pick_a_row_DENSE_protected_0( a_dense, i, vec_row);
	break;
      case CRS_PROTECTED_0:
	Pick_a_row_CRS_protected_0( a_val, a_col, a_rowPtr, i, vec_val_1, vec_index_1, &vec_size_1); break;
      case CRS_PROTECTED_1:
	...
	Pick_a_row_LIL_protected_0( a_row_list, i, vec_val_1, vec_index_1, &vec_size_1); break;
      case LIL_PROTECTED_1:       Pick_a_row_LIL_protected_0( a_row_list, i, vec_val_1, vec_index_1, &vec_size_1); break;
      case JAD_PROTECTED_0:       Pick_a_row_JAD_protected_0( a_JADptr, a_val, a_col, a_row_order, i, vec_val_1, vec_index_1, &vec_size_1, &p_i);
	//printf("jad %d\n", p_i);
	break;
      }
      vec_size_1_pr = max( 0, min( vec_size_1, VEC_MAX ));
      p_i_pr = max(min(p_i, ROWS - 1), 0);//----- protection 0
      /*
      if (i == 0){
	printf("\nrow: %d\n", i);
	printVecI(vec_val_1, vec_size_1_pr);
	printVecI(vec_index_1, vec_size_1_pr);
      ...
      //---- Dot product of a-row[i] and b-vec and update c[i] in dense format
      //spr_vectors_op(vec_val_1, vec_index_1, vec_size_1_pr, b_vec_val, b_vec_index, b_vec_size, vec_val_3, vec_index_3, &vec_size_3_pr, '.');
      //float res = spr_vector_reduction( vec_val_3, vec_index_3, vec_size_3_pr, k, 'S');
	
      float res;
      switch(SPARSE){
      case DENSE_PROTECTED_0:
        spr_vectors_op_DENSE(vec_row, vec_b, vec_res, a_col_max, '.');
        res = spr_vector_reduction_DENSE( vec_res, a_col_max, 'S');
        //printf("res:%f\n", res);
	break;
      default:
	spr_vectors_op(vec_val_1, vec_index_1, vec_size_1_pr, b_vec_val, b_vec_index, b_vec_size, vec_val_3, vec_index_3, &vec_size_3_pr, '.');
        res = spr_vector_reduction( vec_val_3, vec_index_3, vec_size_3_pr, k, 'S');
	//printf("%f\n",res);
        break;
      }
...
start_spr_addr[ Nspr + 1 ] = (int *) &(a_row_list[ 0 ]);
  ...
  int b_vec_val[ROWS], b_vec_index[ROWS], b_vec_size = 0;
  int b_vec [COLS ];
  processLineSprVec(b_vec_val,b_vec_index, &b_vec_size, b_stream);
  fseek(b_stream,0, SEEK_SET);
  processLineVec(b_vec, COLS, b_stream);


  int c_res[ROWS];

  SpMV(a_val, a_col, a_row, a_rowPtr, a_val_col_row, a_val_row, a_index_row, a_row_list, a_rowMax, a_colMax, b_vec_val, b_vec_index, b_vec_size, b_vec, c_res);
  
  int c_file[ROWS];
  processLineVec(c_file, ROWS, ab_stream);

  //printVecI(c_file, ROWS);
  //printf("\n");
  //printVecI(c_res, ROWS);
  
  float rmse = compareVec(c_res, c_file, ROWS);
