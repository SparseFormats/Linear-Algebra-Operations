...
void matrix_diagonal_add(int a_val[], int a_col[], int a_row[], int a_rowPtr[], int a_val_col_row[ NZrowMax + 1 ][ 3 ], int a_val_row[ ROWS ][ NZrowMax ], int a_index_row[ ROWS ][ NZrowMax + 1], val_col_node * a_row_list[], int a_row_max, int a_col_max, int diag_vec_val[], int diag_vec_index[], int diagSize, int vec_diag_add[]){
  ...
  int vec_diag_1[ ROWS ], vec_diag_3[ ROWS ];
  
      //---- Pick diagonal of a
      switch (SPARSE){
      case DENSE_PROTECTED_0:
	Pick_diag_DENSE_protected_0( a_dense, a_row_max , vec_diag_1);
	break;
      case CRS_PROTECTED_0:
	...
	spr_vectors_op_DENSE(vec_diag_1, vec_diag_add, vec_diag_3, ROWS, '+' );
      ...
      */
      ...
      //printVecI(vec_val_3, 10);
      //printf("\n");
      //printVecI(vec_index_3, 10);
      int i2 =0;
      //---- Update diagonal 
	
      switch (SPARSE){
      case DENSE_PROTECTED_0:
	Update_diag_DENSE_protected_0(a_dense, a_row_max, vec_diag_3);
	break;
      case CRS_PROTECTED_0:
	...
	*/
	...
	break;
      case COO_PROTECTED_0:       
	Update_diag_COO_protected_0( a_val_col_row, a_row_max, vec_val_3, vec_index_3, vec_size_3_pr); 
	break;
      case COO_PROTECTED_1:      // Pick_a_row_COO_protected_1( a_val_col_row, i, vec_val_1, vec_index_1, &vec_size_1); 
	break;
      case ELLPACK_PROTECTED_0:
	...
	*/
	...
	break;
      case LIL_PROTECTED_0:       
	  Update_diag_LIL_protected_0( a_row_list, a_LIL_nodes, a_row_max, vec_val_3, vec_index_3, vec_size_3_pr);
	  /*
	    int k1, k2;
	    for (k1 = 0; k1 < (*c_row_max); k1++){
	    printf("\nrow:%d\n",k);
...
void main(){
  ...
  */
  ...
  fseek(diagonal_file, 0, SEEK_SET);
  processLineVec(diag_vec, ROWS, diagonal_file);
  ...
  matrix_diagonal_add(a_val, a_col, a_row, a_rowPtr, a_val_col_row, a_val_row, a_index_row, a_row_list, a_rowMax, a_colMax, diag_val_vec, diag_index_vec, diag_size, diag_vec);
...
switch(SPARSE){
 ...
 case DENSE_PROTECTED_1:
  ...
  case CRS_PROTECTED_0:
  case CRS_PROTECTED_1:
      Sprs_to_dense_CRS(a_val, a_col, a_rowPtr, a_rowMax, a_colMax, c); 
    break; 
  case SLL_PROTECTED_0:
  case SLL_PROTECTED_1:
    Sprs_to_dense_SLL( a_val, a_col, a_row, a_rowMax, a_colMax, c); 
    
    printf("c[0][0]=%d\n", c[0][0]);
    break;
  case COO_PROTECTED_0:
  case COO_PROTECTED_1:
    Sprs_to_dense_COO( a_val_col_row, a_rowMax, a_colMax, c); break;
   break;
