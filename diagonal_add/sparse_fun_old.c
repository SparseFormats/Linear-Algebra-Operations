...
int inject_error(int A[], int **Aptr, val_col_node A_node[], val_col_node **row_list_node , int size_A, double err_probability, int node_type){
  ...
  for (i = 0 ; i < size_A ; i++){
    ...
    // printf("is_err: %d, R: %f, error_probability: %f\n", is_err, R, err_probability);
...
void mem_read_ptrptr_vcn( val_col_node ** dest_address_ptrptr_vcn, val_col_node ** start_address_ptrptr_vcn, int N_address){
  ...
  if (N_address <= 0)
    return;
  err_num += inject_error(NULL,(int **) &start_address_ptrptr_vcn, NULL, NULL, 1, ERR_PR, INT_POINTER);
  
   if (in_spr_bound( (int *) start_address_ptrptr_vcn )){// reconsider
  *dest_address_ptrptr_vcn = *( (val_col_node **)start_address_ptrptr_vcn);
  //    last_read = (int) *( (val_col_node **)start_address_ptrptr_vcn);
    }
    //printf("dest:%d\n",(*dest_address_ptrptr_vcn));
   else{
     printf("IT IS NOT\n\n\n");
     * dest_address_ptrptr_vcn = NULL;//(struct val_col_node *) last_read;
   }//--- it is passed to functional block => error injection
  ///  err_num += inject_error( dest_address_int, NULL, NULL, NULL, N_address, ERR_PR, INT);
}
...
void processLineJAD( int JADptr[], int val[], int col[], int val_row[ ROWS ][ NZrowMax ], int index_row[ ROWS ][ NZrowMax + 1 ], JAD_node sorted_row[], int NZ_row[], int rowMax, int colMax, FILE * stream){
...
void Pick_a_row_SLL_protected_1(int val[], int col[], int row[], int rowNum, int valSpr[], int indexSpr[], int * NZsize){
  int i_start = 0, i_end = 0, NZsize_pr, N_address, row_i_start, row_i_end, row_i_start_pr, row_i_end_pr;
  int *start_address_int;  
  //------ Find i_start
...
void Pick_a_row_ELLPACK_protected_1(int val_row[ ROWS ][ NZrowMax], int index_row[ ROWS ][ NZrowMax + 1 ], int colMax, int rowNum, int valSpr[], int indexSpr[], int * NZsize){
  int i_end = 0, NZsize_pr, N_address, last_element = 0;
  int * start_address_int;
  ...
  while (i_end < NZrowMax){// bc of error
    ...
    //++++ address generator
    start_address_int = &(index_row[ rowNum ][ i_end ]);
    N_address = 1;

    //++++ mem access
    err_num += inject_error(NULL, &start_address_int, NULL, NULL, 1, ERR_PR, INT_POINTER);
    if (inboundry( start_address_int ))
      memcpy( &last_element, start_address_int , N_address * sizeof (int));
    err_num += inject_error( &last_element, NULL, NULL, NULL, N_address, ERR_PR, INT);//--- bc of READ
  ...
  *NZsize = i_end;
  ...
  NZsize_pr = max( 0, min( *NZsize , VEC_MAX ));
  //++++ address generator
  start_address_int = &(val_row[ rowNum ][ 0 ]);
  N_address = NZsize_pr;
  ...
  //++++ address generator
  start_address_int = &(index_row[ rowNum ][ 0 ]);
  N_address = NZsize_pr;
...
void Pick_a_row_LIL_protected_0(val_col_node *row_list [], int rowNum, int valSpr[], int indexSpr[], int * NZsize){
  ...
  //this_node_val= this_node->val;
  // -------------- 2. Copy the row to a sparse vector -------------------//  
   while(this_node != NULL){
  
     if (i_end >= VEC_MAX )
       break;
     //start_address_int = (int *) (*(address_generator_ptr_vcn( 0 , &this_node)));
...
void Pick_a_row_JAD(int JADptr[], float val[], int col[], int rowNum, float valSpr[], int indexSpr[], int * NZsize){

  int i, i_next, j;
  *NZsize = 0;

  //----- Find the size and copy the row in a sparse vector
  for ( j = 0; j < NZrowMax ; j++){
    //--- find the index of item for this row in column j
    i = JADptr[j] + rowNum;
    i_next = JADptr[j + 1];
    if (i >= i_next)//-----  i_next is the next column
      break;
...
void Pick_a_col_JAD(int JADptr[], float val[], int col[], int rowMax, int colNum, float valSpr[], int indexSpr[], int * NZsize){

  int i, j, jc, k = 0;

  for ( i = 0; i < rowMax; i++){ //--- search in all rows
    for ( j = 0; j <= colNum ; j++){
      //--- find the index of item for this column  in row i
      jc = JADptr[j] + i;
      if (col[ jc ] == colNum){
	indexSpr[k] = i;
	valSpr[k] = val[ jc ];
...
void Pick_diag_CRS2(int val[], int col[], int rowPtr[], int rowMax, int valSpr[], int indexSpr[], int *NZsize){
  int i, i_start, i_next, j = 0, k;
...
void Pick_diag_SLL(int val[], int col[], int row[], int rowMax, int valSpr[], int indexSpr[], int * NZsize){
  int k, i = 0;
  for (k = 0; k < NZmax; k++){
    if (row[ k ] >= rowMax)//---- It shows the end of the matrix
      break;
...
void Update_diag_CRS3(int val[], int col[], int rowPtr[], int rowMax, int valSpr[], int indexSpr[], int vecSize){
  int i, i_start, i_next = 0, j = 0, k, v = 0;
  int removed = 0;
  int added = 0;
...
printf("returned : \n");
    printVecI(val, k-removed+added);
    printVecI(col, k-removed+added);
    printVecI(row, k-removed+added);
  */
...
void Update_diag_ELLPACK(int val_row[ ROWS ][ NZrowMax], int index_row[ ROWS ][ NZrowMax + 1 ], int rowMax, int colMax, int valSpr[], int indexSpr[], int vecSize){
  ...
  for (i = 0 ; i < rowMax; i++){
    ...
    //printVecI(val_row[i], NZrowMax);
    //printVecI(index_row[i], NZrowMax + 1);
...
void Remove_a_row_SLL(int val[], int col[], int row[], int rowNum, int rowMax, int valSpr[], int indexSpr[]){
 ...
 while ( row[ i ] == rowNum)
  ...
  memcpy( &(val[ i_start ]), &(val[ i_end ]), (size_matrix - i_start) * sizeof (int));
  memcpy( &(col[ i_start ]), &(col[ i_end ]), (size_matrix - i_start) * sizeof (int));
  memcpy( &(row[ i_start ]), &(row[ i_end ]), (size_matrix + 1 - i_start) * sizeof (int));
}
/*
void Append_a_row_COO(float val_col_row[ ROWS ][ 3 ], int rowNum, float valSpr[], int indexSpr[], int * NZsize){
...
*/
void Remove_a_row_ELLPACK(int val_row[ ROWS ][ NZrowMax], int index_row[ ROWS ][ NZrowMax ], int rowNum, int * rowMax){
...
void Remove_a_col_SLL(int val[], int col[], int row[], int rowMax, int *colMax, int colNum){
...
void Remove_a_col_ELLPACK(int val_row[ ROWS ][ NZrowMax], int index_row[ ROWS ][ NZrowMax + 1 ], int rowMax, int * colMax, int colNum){
  int i, k, removed = 0;
...
void Remove_a_col_LIL(val_col_node *row_list [], val_col_node LIL_nodes[ ROWS ][ NZrowMax + 1 ], int rowMax, int *colMax, int colNum, int valSpr[], int indexSpr[], int vecSize){
