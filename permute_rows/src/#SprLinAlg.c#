...
void add_node2(val_col_node *row_list [], val_col_node LIL_nodes[ ROWS ][ NZrowMax + 1], int row_id, int col_id, int in_data){
...
/*
void add_node1(val_col_node *row_list [], val_col_node LIL_nodes[ ROWS ][ NZrowMax + 1], int row_id, int col_id, int in_data){
  int nz_row = 0;
  val_col_node ** start_address_ptrptr_vcn;
  val_col_node * this_row_list;
  val_col_node * this_node, *this_node_next;
...
void Append_a_row_LIL_protected_0(val_col_node *row_list [], val_col_node LIL_nodes[ ROWS ][ NZrowMax + 1 ], int valSpr[], int indexSpr[], int vec_size, int* rowMax){
  ...
  for(k = 0; k < vec_size; k++)
    add_node1(row_list, LIL_nodes, *(rowMax), indexSpr[ k ], valSpr[ k ]);
...
void matrix_multiplication(int a_val[], int a_col[], int a_row[], int a_rowPtr[], int a_val_col_row[ NZrowMax + 1 ][ 3 ], int a_val_row[ ROWS ][ NZrowMax ], int a_index_row[ ROWS ][ NZrowMax + 1], val_col_node * a_row_list[], int a_row_max, int a_col_max, int b_val[], int b_col[], int b_row[], int b_rowPtr[], int b_val_col_row[ NZrowMax + 1 ][ 3 ], int b_val_row[ ROWS ][ NZrowMax ], int b_index_row[ ROWS ][ NZrowMax + 1], val_col_node * b_row_list[], int b_row_max, int b_col_max, int **c){
  ...
  int vec_size_1_pr, vec_size_2_pr, vec_size_3_pr;
	...
	case LIL_PROTECTED_0:       
	  Pick_a_row_LIL_protected_0( a_row_list, i, vec_val_1, vec_index_1, &vec_size_1); break;
...
void matrix_permute_rows(int a_val[], int a_col[], int a_row[], int a_rowPtr[], int a_val_col_row[ NZrowMax + 1 ][ 3 ], int a_val_row[ ROWS ][ NZrowMax ], int a_index_row[ ROWS ][ NZrowMax + 1], val_col_node * a_row_list[], int a_row_order[], int a_row_max, int a_col_max, int c_val[], int c_col[], int c_row[], int c_rowPtr[], int c_val_col_row[ NZrowMax + 1 ][ 3 ], int c_val_row[ ROWS ][ NZrowMax ], int c_index_row[ ROWS ][ NZrowMax + 1], val_col_node * c_row_list[], int *c_row_max, int *c_col_max, int perm_order[]){
  int i, p_i;
  
  if (SPARSE == JAD_PROTECTED_0){
    permute_rows_JAD(a_row_order, perm_order);
    return;
  }
  ...
  int vec_row_1[ COLS];
	...
	Pick_a_row_DENSE_protected_0( a_dense, p_i, vec_row_1);
	break;
     case CRS_PROTECTED_0:
	...
	break;
      case SLL_PROTECTED_1:   
	Pick_a_row_SLL_protected_1( a_val, a_col, a_row, p_i, a_row_max, a_col_max, vec_val_1, vec_index_1, &vec_size_1);
	break;
      case COO_PROTECTED_0:       
	Pick_a_row_COO_protected_0( a_val_col_row, p_i, vec_val_1, vec_index_1, &vec_size_1); 
	break;
      case COO_PROTECTED_1:        
	Pick_a_row_COO_protected_1( a_val_col_row, p_i, a_row_max, a_col_max, vec_val_1, vec_index_1, &vec_size_1); 
	break;
      case ELLPACK_PROTECTED_1:   
	Pick_a_row_ELLPACK_protected_1( a_val_row, a_index_row, a_col_max, p_i, a_row_max, vec_val_1, vec_index_1, &vec_size_1); 
	break;
      case ELLPACK_PROTECTED_0:   
	...
	case JAD_PROTECTED_0:       
	  Pick_a_row_JAD( a_JADptr, a_val, a_col, p_i, vec_val_1, vec_index_1, &vec_size_1);
	//printf("i: %d\n",i);
	break;
      }
      vec_size_1_pr = max( 0, min( vec_size_1, VEC_MAX ));
      ...
      if (i == 0){
      printf("\nrow: %d\n", i);
      if (SPARSE==DENSE_PROTECTED_0)
	printVecI(vec_row_1,COLS);
      else{
	printVecI(vec_val_1, vec_size_1_pr);
	printf("index\n");
	printVecI(vec_index_1, vec_size_1_pr);
	...
	*c_col_max = a_col_max;
	switch (SPARSE){
	case DENSE_PROTECTED_0:
	  Append_a_row_DENSE_protected_0(c_dense, vec_row_1, c_row_max);
	  break;
	case CRS_PROTECTED_0:
...
start_spr_addr[ Nspr + 1 ] = (int *) &(a_row_list[ 0 ]);
  ...
  */
  ...
  int per_order[ROWS];
  processLineVec(per_order, ROWS, per_file);

  matrix_permute_rows(a_val, a_col, a_row, a_rowPtr, a_val_col_row, a_val_row, a_index_row, a_row_list, a_row_order, a_rowMax, a_colMax, c_val, c_col, c_row, c_rowPtr, c_val_col_row, c_val_row, c_index_row, c_row_list, &c_rowMax, &c_colMax, per_order);
  //printVecI(c_dense[0], COLS);
switch(SPARSE){
  case DENSE_PROTECTED_0:
  case DENSE_PROTECTED_1:
 ...
 */ 
 ...
 case DENSE_PROTECTED_1:
   strcpy(sparseStr,"DENSE_PROTECTED_1"); break;
 case CRS_PROTECTED_0:
